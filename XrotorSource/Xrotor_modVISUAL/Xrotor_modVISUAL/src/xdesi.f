...
c     se non viene specificato alcun valore di CL viene posto a +-0.8 a seconda del segno di Thrust (o Power)
        IF(DEST) CLDES1 = SIGN(0.8,TDDES)
        IF(DESP) CLDES1 = SIGN(0.8,PDDES)
        CALL SETCLD(CLDES1,CLDES1)
      ELSE
        CALL SETCLD(CLDES0,CLDES0) ! se viene specificato CL costante si interpola tra due valori uguali
      ENDIF
C
...
C
      ELSEIF(ANS.EQ.'CL  ') THEN
        ...
        CLDES0 = 0.5*(CLROOT+CLTIP) !viene stampato come valore indicativo in CL costante la media dei due valori immessi
...
C---- Smooth filter the GAM for low relaxation factors
        IF(RLX.LT.0.2) THEN
          IF (stampa_schermo)  WRITE(*,*) 'DEITER filtering GAM'
