...
C
C---------------------------------------------------------------------
C--- Write current prop operating point data to file ATTENZIONE: non è il case file!!!
   20 IF(COMARG(1:1).NE.' ') SAVFIL = COMARG
      CALL OPFILE(LUSAVE,SAVFIL) !chiede il nome del file su cui salvare
      CALL OUTPUT(LUSAVE) !stampa l'output della pala (non il casefile!!!)
      CLOSE(LUSAVE)
      GO TO 900
C
C--------------------------------------------------------------
...
C--- Change altitude
   35 IF(NINPUT.GE.1) THEN
      ...
      IF (stampa_schermo) CALL FLOSHO(LUWRIT, VSO, RHO, RMU)
      GO TO 900
C
C---------------------------------------------------------------------
...
ccc     &                    POW,THR,TRQ,EFF
        ...
        N = 13
        CALL GETFLT(LINE,A,N,ERROR) !Acquisizione dati
        IF(ERROR) GO TO 99
...
C
 1000     CONTINUE
C
      IF (filterGAM.GT.0) WRITE(*,*) 'APITER filtering GAM',filterGAM
      RETURN
      END ! APITER
