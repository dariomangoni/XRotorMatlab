...
C
 910  IF    (NPLOT.EQ.0) THEN
       GO TO 900
	  ELSE
	   WRITE(*,*) 'hai selezionato il plot inesistente, ottima scelta'
      ENDIF
      GO TO 900
C
C---------------------------------------------------------------------
C--- Annotate plot
 92   WRITE(*,*) 'niente plot niente annotazioni'
c     IF(LPLOT) THEN
c       CALL ANNOT(1.2*CSIZE)
c      ELSE
...
C--- Hardcopy current plot
 94   WRITE(*,*) 'oops hardcopy è fiammata via'
c IF(LPLOT) THEN
c       CALL PLEND
...
C------------------------------------------------
      WRITE(*,*) 'Modifica da puntatore disabilitata '
C
      RETURN
      END
