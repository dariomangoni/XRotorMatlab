...
C
C-------------------------------------------------------------
 62   WRITE(*,*) 'Funzione disabilitata'
      GO TO 900
C
C-------------------------------------------------------------
 65   WRITE(*,*) 'Funzione disabilitata'
      GO TO 900
C
C-------------------------------------------------------------
...
C..............................
 ...
 3100 FORMAT(/' Blade mass =', F9.3
     &       /' Pitch-axis inertia =',F12.5
     &       /' Rotational inertia =',F9.3)
      END ! EILOAD
...
C
c        IF(MOMREF .LT. 1.0) THEN
c         WRITE(LU,1030) I,XI(I),WXA,WZA,TYA,MZA,MXA,MYA,SYA
c        ELSE IF(MOMREF .LT. 10.0) THEN
c         WRITE(LU,1031) I,XI(I),WXA,WZA,TYA,MZA,MXA,MYA,SYA
c        ELSE IF(MOMREF .LT. 100.0) THEN
c         WRITE(LU,1032) I,XI(I),WXA,WZA,TYA,MZA,MXA,MYA,SYA
c        ELSE 
c         WRITE(LU,1033) I,XI(I),WXA,WZA,TYA,MZA,MXA,MYA,SYA
c        ENDIF
C
        WRITE(LU,1035) I,XI(I),WXA,WZA,TYA,MZA,MXA,MYA,SYA
C        
   10 CONTINUE
C
C
...
C
 1020 FORMAT(
     ...
     & /'                          (deg)      (N-m)       (N-m)',
     &  '       (N-m)       (N)  ')
C         10  0.425  0.152  0.124  -7.02  1345.40  1262.43  1234.02
C          11234.0    
 1030 FORMAT(1X,
     &    I2, F7.3,  F7.3,  F7.3,   F7.2, 1x,F9.5, 1x,F9.5,  1x,F9.5,
     &     F9.4 )
 1031 FORMAT(1X,
     &    I2, F7.3,  F7.3,  F7.3,   F7.2, 1x,F9.4, 1x,F9.4,  1x,F9.4,
     &     F9.3 )
 1032 FORMAT(1X,
     &    I2, F7.3,  F7.3,  F7.3,   F7.2, 1x,F9.3, 1x,F9.3,  1x,F9.3,
     &     F9.2 )
 1033 FORMAT(1X,
     &    I2, F7.3,  F7.3,  F7.3,   F7.2, 1x,F9.2, 1x,F9.2,  1x,F9.2,
     &     F9.1 )
 1035 FORMAT(1X,
     &    I2, F7.3,  F7.3,  F7.3,   F7.2, 1x,G12.4, 1x,G12.4,  1x,G12.4,
     &     G12.4 )
C
C
 2020 FORMAT(
...
C          10  0.425  10.002  14.002  20.203  12.000  13.450
 2030 FORMAT(1X,
     &     I2, F7.3,  F8.3,   F8.3,   F8.3,   F8.3,   F8.3  )
C
      END ! STWRIT
