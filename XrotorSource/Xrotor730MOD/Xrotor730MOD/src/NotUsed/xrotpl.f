...
C---- blade angles in degrees
      BHUB = BETA(1)  * 180.0/PI
      BTIP = BETA(II) * 180.0/PI
      BTWIST = BHUB - BTIP
C---- twist defined to blade hub
c      B2   = BETA(2)  * 180.0/PI
...
C
      ...
      IF(WIND) THEN
       ...
       CALL XYLINE(II,XI,W3,0.0,1.0,0.0,-WFAC,2)
      ELSE
       CALL XYLINE(II,XI,W1,0.0,1.0,0.0, WFAC,1)
       CALL XYLINE(II,XI,W3,0.0,1.0,0.0, WFAC,2)
      ENDIF
...
C
      ...
      CALL NEWPEN(3)
      CALL PLCHAR(XL,YL,CSL,'On blade',0.0,8)
C
      YL = YL - 2.0*CSL
      CALL NEWPEN(4)
      CALL XYLINE(2,W8,W9,-XL,1.0,-YL,1.0,2)
      CALL NEWPEN(3)
      CALL PLCHAR(XL,YL,CSL,'Average',0.0,7)
C
      CALL PLFLUSH
C
      RETURN
...
C
      ...
      INCLUDE 'XROTOR.INC'
C--------------------------------------------
C     Plots imposed slipstream v/V vs r/R
C--------------------------------------------
      EXTERNAL PLCHAR,PLMATH
      DATA LMASK1, LMASK2, LMASK3 / -32640, -30584, -21846 /
C
...
C
      ...
      ENDDO
C
      CALL SCALIT(1,YMAX,YMIN,YSF1)
C
      YDEL = MAX( 1.0/(5.0*YSF1) , 0.01 )
      YMIN = YDEL*(AINT(YMIN/YDEL + 1000.99) - 1001.0)
      YMAX = YDEL*(AINT(YMAX/YDEL + 1001.01) - 1000.0)
      YSF  = PLPAR / (YMAX-YMIN)
C
      XDEL = 0.2
      XMIN = 0.0
...
C
      CALL NEWPEN(4)
      YLINE = YMAX*YSF + 5.0*CSL
      CALL PLCHAR(0.0,YLINE,1.2*CSL,NAME,0.0,31)
C
      YLINE = YMAX*YSF + 2.5*CSL
      CALL PLCHAR(0.0,YLINE,CSL,'Imposed velocity perturbations',0.0,-1)
C
      IF(NADD.GT.0) THEN
       IL = NADD/4 + 1
       CALL NEWCOLORNAME('red')
...
C
       ...
       CALL PLCHAR(XPLT,YPLT,1.2*CSL,'v  /V',0.0,5)
       CALL PLSUBS(XPLT,YPLT,1.2*CSL,'axi'  ,0.0,3,PLCHAR)
C
       IL = NADD/3 + 1
...
C
       ...
       CALL PLCHAR(XPLT,YPLT,1.2*CSL,'v  /V',0.0,5)
       CALL PLSUBS(XPLT,YPLT,1.2*CSL,'tan'  ,0.0,3,PLCHAR)
      ENDIF
C
      CALL NEWCOLOR(ICOL0)
C
...
C---- blade angles in degrees
      BHUB = BETA(1)  * 180.0/PI
      B2   = BETA(2)  * 180.0/PI
      BEXT = BHUB - XI(1)*(BHUB-B2)/(XI(1)-XI(2))
      BTIP = BETA(II) * 180.0/PI
      BTWIST = BEXT - BTIP
C
C---- character size for axis numbers, labels
...
C------ plot blade shape
        ...
        DO IB=1, NBLDS
          ...
          COSA = COS(ANG)
          DO I = 1, II
            XP = XI(I)*COSA - W3(I)*SINA
            YP = XI(I)*SINA + W3(I)*COSA
            IF(I.EQ.1) THEN
              CALL PLOT(XP,YP,3)
             ELSE
              CALL PLOT(XP,YP,2)
          ...
          ENDDO
          DO I = 1, II
            XP = XI(I)*COSA - W4(I)*SINA
            YP = XI(I)*SINA + W4(I)*COSA
            IF(I.EQ.1) THEN
              CALL PLOT(XP,YP,3)
             ELSE
              CALL PLOT(XP,YP,2)
