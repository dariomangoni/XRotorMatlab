...
C
      INCLUDE 'XROTOR.INC'
C
      LOGICAL LERROR, LXRELIMIT, LYRELIMIT, LEGEND2D
      LOGICAL LPLT_DATA
      CHARACTER*4 OPT, XVAR, YVAR
      CHARACTER PLTTYPE*(*)
...
C---- Change Ordinate
 ...
 55     FORMAT(/' ================================================'
     &       /' Ordinate is: ',A,
     &       /'   CH, BE, GAM, CL, CD, RE, EFP, VA, VT, VW, XW, UB',
     &       /'   Ti, Pi, Tv, Pv, Tw, Pw',
     &      //' Select option (or <return>):  ', $)
        READ(*,1,ERR=50) OPT
        CALL LC2UC(OPT)
...
C--- Install Y data
          IF(YVAR.EQ.'CH') THEN
           ...
           ELSEIF(YVAR.EQ.'VA') THEN
            ...
            ENDIF
            YDATA(I,ND) = VIND(1,I)*VEL * VADUCT_VA
