...
C--- Hardcopy current plot
    6 WRITE(*,*) 'Funzione disabilitata'
      GO TO 900
C
C---------------------------------------------------------------------
C--- Change plot size
    8 WRITE(*,*) 'Funzione disabilitata'
      GO TO 900
C
C---------------------------------------------------------------------
C--- Annotate plot
    9 WRITE(*,*) 'Funzione disabilitata'
      GO TO 900
C
C---------------------------------------------------------------------
...
C---------------------------------------------------------------------
C--- Plot stuff
   30 WRITE(*,*) 'Funzione disabilitata'
      GO TO 900
C
C---------------------------------------------------------------------
C--- Change altitude
...
C
      ...
      IF(FNAME(1:1).NE.' ') THEN
        ...
        END DO
        WRITE(*,*) ' '
        CALL SPLINE(PWRVAR,XPWRVAR,RPMVAR,NPWRVAR)
        CLOSE(LU)
        LPWRVAR = .TRUE.
      ENDIF     
C
...
C----- set up equation to converge wake advance ratio
       ...
       END DO
       Q(K2,K1) = Z_TI*TI_ADV + Z_PI*PI_ADV - 1.0
...
C---- limit GAM changes that change sign
        IF(DGAM(I)*DGAMOLD(I) .LT. 0.0) THEN
          IF(ABS(DGAM(I)).GT.0.2*ABS(DGAMOLD(I))) THEN
            RLX = MIN(RLX,0.2)
ccc        write(*,998) 'DGAM limiter i,rlx,gam,dgam ',i,rlx,gam(i),dgam(i)
          ENDIF
        ENDIF
C
      ENDDO
...
C    for this iteration
         IF(CHNEW.LE.0.0) THEN
           write(*,*) 'TPQ negative chord @I = ',I,CHNEW
           IF(I.EQ.1) THEN
            CH(I)  = CH(I+1)
           ELSEIF(I.EQ.II) THEN
            CH(I)  = CH(I-1)
...
C       Assumes tip speed and gamma are same in real and equivalent prop
C
        VW = VWAK(I)
        UTOTW = 1.0
        CALL UVADD(XI(I),WA,WT)
C------ Cw( Adv , Vw )      ;  Vw( Gj , Adv , Adw )
        CW     =  XI(I)/ADV - WT  -  VW
        CW_ADV = -XI(I)/ADV**2
        CW_VW  =                  -  1.0
        CW_XW  =  0.0
...
C
      ELSE
C
C----- for free-tip case, incorporate Prandtl's averaging factor F
       SFAC = SQRT(1.0 + 1.0/ADW**2)
       SF_ADW = 0.5/SFAC * (-2.0/ADW**3)
C
       DO 20 I=1, II
...
C
      ...
      SUBROUTINE SHOCASMOD(LU,NDIM,N,PAR,RAD,NAME)
      DIMENSION PAR(0:NDIM,*)
      CHARACTER NAME*(*)
C
      IF(NDIM.LT.11) THEN
...
C
      ...
      SUBROUTINE GETCASMOD(LU,NDIM,NCAS,PAR)
      DIMENSION PAR(0:NDIM,*), A(16)
      CHARACTER DUMMY*1, LINE*256, CNAME*32
      LOGICAL ERROR
C
      IF(NDIM.LT.11) THEN
        WRITE(*,*) 'Error in GETCAS: NDIM too small for PAR array'
...
C
      PI = 4.0*ATAN(1.0)
C------ Salva il nome del casefile e salta le righe di intestazione
 1000 FORMAT(A)
      READ(LU,1000) CNAME
cc      WRITE(*,*) 'Case name: ',CNAME
      READ(LU,1000) DUMMY
      READ(LU,1000) DUMMY
C------- Inizia lo 'scanning' dei diversi case file
      DO I=1, 12345
        READ(LU,1000,END=11) LINE
        N = 13
        CALL GETFLTMOD(LINE,A,N,ERROR) !Acquisizione dati
        IF(ERROR) GO TO 99

C--- Set parameters for cases
        PAR(1,I) = A(2)               !Advance ratio
        PAR(2,I) = A(3)*PI/180.0      !Blade angle converted in radians
        PAR(3,I) = A(4)               !Airspeed[m/s]
        PAR(4,I) = A(5)               !Rpm
        PAR(5,I) = A(6)               !Rho
        PAR(6,I) = A(7)/1.0E5         !Rmu cioè mu (*10e5)
        PAR(7,I) = A(8)               !Sound speed[m/s]
        PAR(8,I) = A(9)               !Altitude[km]
        PAR(9,I) = A(10)              !Power[W]
        PAR(10,I)= A(11)              !Thrust[N]
        PAR(11,I)= A(12)              !Torque[Nm]
        PAR(12,I)= A(13)              !Efficiency (obviously 0 during reading)
      ENDDO
 11   CONTINUE
      NCAS = I-1
      RETURN
...
C--- Read case accumulation arrays from saved file
      ...
      CALL GETCASMOD(LU,NPARX,NCASE,CASPAR)
      
      DO ICASE=1, NCASE
          RPMFIX = .FALSE.
          PITCHFIX = .FALSE.
          IF (CASPAR(3,ICASE).LE.0) GO TO 3      !se la velocità è minore o uguale a zero passa al case successivo
...
c----- se non si ottengono i dati atmosferici li calcoli in base alla quota
          IF(RHO.EQ.0) THEN
              IF(RMU.EQ.0) THEN
                  IF(VSO.EQ.0) CALL ATMO(ALT,VSO,RHO,RMU)
              ENDIF
          ENDIF
     
c----- se non è fissato l'angolo di pala significa che si vuole fissare P o T o Q con RPM costante     
          IF (BET.EQ.(999.0*PI/180)) THEN          
              RPMFIX = .TRUE.
              GO TO 8
          ENDIF

C----- se non è fissato RPM significa che si vuole fissare P o T o Q con PITCH costante
          IF (RPM.EQ.0) THEN              !
              PITCHFIX = .TRUE.
              GO TO 8
          ENDIF
      
c---- Funzionamento diretto - se si arriva qui vuol dire che si vuole calcolare P,T,Q conoscendo RPM e blade angle
...
c          IF(CONV) THEN
c           CASPAR(9,ICASE) = PTOT*RHO*VEL**3*RAD**2
c           CASPAR(10,ICASE) = TTOT*RHO*VEL**2*RAD**2
c           CASPAR(11,ICASE) = QTOT*RHO*VEL**2*RAD**3
c           CASPAR(12,ICASE) = TTOT/PTOT
c          ENDIF
          GO TO 4
c---- Fine funzionamento diretto
      
      
c---- Is specified Thrust or Power or Torque?
8         IF (THR.NE.999) GO TO 50
          IF (TRQ.NE.0) GO TO 60
          IF (POW.NE.0) GO TO 70
...
50    TSPEC = THR;
      CONV = .FALSE.
      BSAV = BETA(II)
      IF (PITCHFIX) CALL APER(1,2,LOPRINI)
      IF (RPMFIX) CALL APER(1,1,LOPRINI)
C
C---- Check for valid blade angle change 
      IF (.NOT.PITCHFIX .AND. .NOT.CONV) THEN
C----- convergence failed: restore clobbered blade angles
        DO I=1, II
         BETA(I)  = BETA(I)  - DBETA
         BETA0(I) = BETA0(I) - DBETA
...
C------Specified Torque ---------------------------------------
   60 QSPEC = TRQ
      CONV = .FALSE.
      IF(PITCHFIX) CALL APER(2,2,LOPRINI)
      IF(RPMFIX) CALL APER(2,1,LOPRINI)
...
C--- Specify power and solve
   70 PSPEC = POW
      CONV = .FALSE.
      IF(PITCHFIX) CALL APER(3,2,LOPRINI)
      IF(RPMFIX) CALL APER(3,1,LOPRINI)
C
C---- Check for valid blade angle change 
      IF (.NOT.PITCHFIX .AND. .NOT.CONV) THEN
C----- convergence failed: restore clobbered blade angles
 ...
 4    IF (PITCHFIX) RPM  = VEL/(RAD*ADV*PI/30.)
      CASPAR(1,ICASE) = ADV
      CASPAR(2,ICASE) = BETA(II)
      CASPAR(3,ICASE) = VEL
      CASPAR(4,ICASE) = RPM
      CASPAR(5,ICASE) = RHO
      CASPAR(6,ICASE) = RMU
      ...
      CASPAR(8,ICASE) = ALT
      IF (CONV) THEN
      CASPAR(9,ICASE) = PTOT*RHO*VEL**3*RAD**2
      CASPAR(10,ICASE) = TTOT*RHO*VEL**2*RAD**2
      CASPAR(11,ICASE) = QTOT*RHO*VEL**2*RAD**3
      CASPAR(12,ICASE) = TTOT/PTOT
      ENDIF
 ...
 3    CONTINUE
      ...
      OPEN(LU,FILE=FILENAME,STATUS='OLD',ERR=99)
      CALL SHOCASMOD(LU,NPARX,NCASE,CASPAR,RAD,NAME)
      CLOSE(LU)
      RETURN
      
C
 99   WRITE(*,*) 'OPEN error on file'
      RETURN
      ...
      SUBROUTINE AUTOOPERITER(FILENAME)
      INCLUDE 'XROTOR.INC'
      LOGICAL ERROR
      LOGICAL RPMFIX
      LOGICAL PITCHFIX
      CHARACTER*80 FILENAME
      REAL OSCILL_RESULT, OSCILL_MAX, LAST_VALUE
      OSCILL_MAX = 0.025
...
C---- if pitch could be changed and if convergence failed restore previous blade angle
      IF (.NOT.PITCHFIX .AND. .NOT.CONV) THEN
        DO I=1, II
         BETA(I)  = BETA(I)  - DBETA
         BETA0(I) = BETA0(I) - DBETA
...
c---  Save data to case array
 ...
 3    IF (PITCHFIX .OR. RPMFIX) THEN
          OSCILL_RESULT = (LAST_VALUE-BETA(II))/BETA(II)
          LAST_VALUE = BETA(II)
          IF (ABS(OSCILL_RESULT).LT.OSCILL_MAX) THEN
              GO TO 5
          ELSE
              IF (TSPEC.NE.999) GO TO 50
              IF (QSPEC.NE.0) GO TO 60
              IF (PSPEC.NE.0) GO TO 70
          ENDIF
      ENDIF
      
      GO TO 5
c     il funzionamento diretto dovrebbe essere già pressochè corretto, non c'è bisogno di iterare     
      
 9    CONTINUE    !eventualmente utile se si dovessero attuare accorgimenti per evitare che, in seguito ad uno sbiello, i punti successivi possano essere calcolati
