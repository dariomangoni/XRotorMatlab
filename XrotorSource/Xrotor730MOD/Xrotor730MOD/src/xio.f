...
C--- Save blade definition with chord,twist and body velocity
      DO I=1, II
        BETA0DEG = BETA0(I)*180.0/PI
        WRITE(LU,1200) XI(I),CH(I),BETA0DEG,UBODY(I),CL(I)
      END DO
      
c-- Save more detailed blade definition using OUTPUT with LU as file
      
C--- Duct velocity
      WRITE(LU,1113)
      WRITE(LU,1200) URDUCT
      
c--- Print some other useful data
      ...
      WRITE(LU,1300)
      WRITE(LU,1310) TDIM, QDIM, PDIM, RPM, EFFTOT
...
C...................................................................
 ...
 1111 FORMAT('!LVDuct  LDuct   LWind'/3(1X,L2,5X))
 1112 FORMAT('!   II Nblds'/2(1X,I5),
     &      /'!         r/R          C/R     Beta0deg     Ubody     CL')
 1113 FORMAT('!      URDuct')
 1114 FORMAT('!Nadd'/1(1X,I5),
     &      /'!        Radd         Uadd         Vadd')
 1200 FORMAT(6(1X,G12.5))
 1300 FORMAT('! Thrust[N]   Torque[Nm]   Power[W]   rpm     Efficiency')
 1310 FORMAT(5(1X,G12.5))
...
C--- Check for new format/old format XROTOR file
      ...
      ENDIF
      READ(LINE(17:22),*) FILEVERS
      if (stampa_schermo) WRITE(*,1005) FILEVERS
C
C
C--- Case title
      CALL RDLINE(LU,LINE)
...
C---- calculate current operating point
      CALL APER(4,2,.TRUE.)
      IF((CONV).and.stampa_schermo) CALL OUTPUT(LUWRIT)
C
C---- define design quantities for design of MIL prop with same parameters
      RADDES = RAD
      VELDES = VEL
...
C--- Impose the number of analysis stations to use
      II = IIX
      
C--- Optional(really???) duct velocity
18    URDUCT = 1.0
      CALL RDLINE(LU,LINE)
      IF(LINE.EQ.'END' .OR. LINE.EQ.'ERR') GO TO 19       
      READ(LINE,*,END=19) URDUCT
      
c--- Load design point data
      CALL RDLINE(LU,LINE)
      IF(LINE.EQ.'END' .OR. LINE.EQ.'ERR') GO TO 19       
      READ(LINE,*,END=191) TDDES, PDDES
      IF (TDDES.EQ.0.0) THEN
          IF (PDDES.EQ.0.0) THEN
              IF (stampa_schermo) GO TO 191
              GO TO 21
          ELSE
              DEST = .FALSE.
              DESP = .TRUE.
      ...
      CALL RDLINE(LU,LINE)
      IF(LINE.EQ.'END' .OR. LINE.EQ.'ERR') GO TO 19       
      READ(LINE,*,END=191) CLROOT, CLTIP
      IF (CLROOT.EQ.999.0) THEN
       IF (stampa_schermo) GO TO 191
      ELSE
          CALL SETX
          CALL SETCLD(CLROOT,CLTIP)
          CLDES0 = (CLROOT+CLTIP)*0.5
      ENDIF
...
C
C---- define design quantities for design of MIL prop with same parameters
      RADDES = RAD !kept from file
      VELDES = VEL !kept from file
      ADVDES = ADV !in any case in DESI>EDIT will appear the rotor as RPM specified so ADV ratio is 0
      RPMDES = VEL/(RAD*ADV) * 30.0/PI !kept from file
      R0DES = XI0*RAD !kept from file
      RWDES = XW0*RAD !kept from file
...
C
  ...
  191 WRITE(*,*) 'qualcosa non va nelle mie modifiche'
      CLOSE(LU)
      CONV = .FALSE.
      RETURN
C..............................
